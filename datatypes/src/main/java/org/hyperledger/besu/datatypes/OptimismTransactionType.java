...
public enum OptimismTransactionType implements TransactionType {
  ...
  private static final Set<TransactionType> LEGACY_FEE_MARKET_TRANSACTION_TYPES =
      Set.of(OptimismTransactionType.FRONTIER, OptimismTransactionType.ACCESS_LIST);

  private final int typeValue;

  OptimismTransactionType(final int typeValue) {
  ...
  public static TransactionType of(final int serializedTypeValue) {
    return Arrays.stream(
            new TransactionType[] {
              OptimismTransactionType.FRONTIER,
              OptimismTransactionType.ACCESS_LIST,
              OptimismTransactionType.EIP1559,
              OptimismTransactionType.BLOB,
              OptimismTransactionType.OPTIMISM_DEPOSIT,
              OptimismTransactionType.DELEGATE_CODE
            })
